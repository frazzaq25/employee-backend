...
exports.changePassword = catchAsyncError(async (req, res, next) => {
  ...
  const user = await Employee.findById(req.employee._id).select("+password");
...
exports.forgetPassword = catchAsyncError(async (req, res, next) => {
  ...
  const resetToken = await employee.getResetToken();
  await employee.save();
  ...
  res.status(200).json({
      success: true,
      message:`Reset token has been sent to ${employee.email}`
  })
})
